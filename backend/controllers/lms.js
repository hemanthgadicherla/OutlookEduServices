...
// Bunny helper — loaded lazily so missing keys don't crash startup
const getBunny = () => require('../utils/bunny');
  ...
  try {
    ...
    const { data: allProgress } = allLessonIds.length > 0
      ? await supabase.from('lesson_progress')
          ...
          .eq('user_id', userId).eq('completed', true).in('lesson_id', allLessonIds)
      : { data: [] };
    ...
    const data = unique.map(course => {
      ...
      return { course_id: course.id, payment_status: 'paid', enrolled_at: reg?.created_at || null, progress, total_lessons: total, completed_lessons: completed, courses: course };
    });
...
const getStats = async (req, res) => {
  try {
    ...
    const { data: allProgress } = await supabase
      .from('lesson_progress').select('lesson_id, completed, watched_seconds').eq('user_id', userId);

    const completedLessons = (allProgress || []).filter(p => p.completed).length;
    const timeSpentSeconds = (allProgress || []).reduce((s, p) => s + (p.watched_seconds || 0), 0);

    const { data: recentProgress } = await supabase
      .from('lesson_progress').select('completed_at')
    ...
    const weeklyActivity = Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        ...
        lessons: (recentProgress || []).filter(p => p.completed_at?.startsWith(dateStr)).length
      };
    });
    ...
    return res.json({
      success: true,
      data: {
        total_purchased:   paidRegs?.length || 0,
...
const getCertificates = async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('certificates').select('*, courses(title, image)')
      .eq('user_id', req.user.id).order('issued_at', { ascending: false });
    if (error) throw error;
...
// ─────────────────────────────────────────────────────────────
// GET /api/lms/lesson/:id/stream
// Returns signed Bunny embed URL or plain URL for a lesson.
// Enforces paid access for locked lessons.
// ─────────────────────────────────────────────────────────────
const getStreamUrl = async (req, res) => {
  try {
    ...
    const { data: lesson } = await supabase
      .from('course_lessons')
      .select('id, title, video_url, is_free, module_id')
      .eq('id', lessonId).maybeSingle();

    if (!lesson) return res.status(404).json({ success: false, message: 'Lesson not found' });

    // Enforce paid access for locked lessons
    if (!lesson.is_free) {
      ...
      if (mod) {
        ...
        if (!paidIds.includes(mod.course_id))
          return res.status(403).json({ success: false, message: 'Purchase this course to access this lesson.' });
      }
    }

    const videoUrl = lesson.video_url;

    // Bunny Stream — signed embed URL (hides API key from client)
    if (videoUrl?.startsWith('bunny:')) {
      const videoId = videoUrl.replace('bunny:', '');
      const { getBunnySignedEmbedUrl } = getBunny();
      const { url, expires } = getBunnySignedEmbedUrl(videoId, 14400); // 4 hours
      return res.json({ success: true, type: 'bunny', url, expires });
    }

    // YouTube / direct URL — return as-is
    if (videoUrl) {
      return res.json({ success: true, type: 'url', url: videoUrl });
    }

    return res.json({ success: true, type: 'none', url: null });
  } catch (err) {
...
// ─────────────────────────────────────────────────────────────
// GET /api/lms/resume
// Last-touched incomplete lesson for "Continue Learning" card
// ─────────────────────────────────────────────────────────────
const getResume = async (req, res) => {
  try {
    ...
    const { data: latest } = await supabase
      .from('lesson_progress')
      .select('lesson_id, watched_seconds, updated_at')
    ...
    const { data: lesson } = await supabase
      .from('course_lessons').select('id, title, module_id').eq('id', latest.lesson_id).maybeSingle();
    ...
    const { data: course } = await supabase
      .from('courses').select('id, title, image, category').eq('id', mod.course_id).maybeSingle();
...
// ─────────────────────────────────────────────────────────────
// GET /api/lms/lesson/:id/video-url  (alias for stream)
// ─────────────────────────────────────────────────────────────
const getLessonVideoUrl = async (req, res) => {
  return getStreamUrl(req, res);
};
...
const getCourseSuggestions = async (req, res) => {
  try {
    ...
    if (courseNames.length > 0) {
      const { data } = await supabase
        .from('courses').select('id, title, description, image, category')
        .in('title', courseNames).neq('id', courseId);
      if (data) others = [...others, ...data];
    }
